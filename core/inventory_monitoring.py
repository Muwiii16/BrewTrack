...
def build_header():
    breadcrumb = ft.Row([
        ...
        ft.Text("Inventory Monitoring", size=14, color=TEXT_PRIMARY),
    ], spacing=4,)
...
def build_stats_row():
    ...
    inventory_value = sum(float(i["value"]) for i in inventory_model.get_inventory_overview())
    low_or_out = sum(
        1 for i in inventory_model.get_inventory_overview() if float(i["current_quantity"]) <= float(i["reorder_level"])
    )
    out_of_stock = sum(1 for i in inventory_model.get_inventory_overview() if float(i["current_quantity"]) <= 0)
...
def inventory_monitoring_view(page: ft.Page, user, on_logout, on_nav):
    sidebar = build_sidebar(page, user, on_logout, "Inventory Monitoring", on_nav)
