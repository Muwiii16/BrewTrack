...
def build_ingredients_supply_table():
    db_items = ingredient_model.get_ingredients()
    
    rows = [table_header_row()]
    
    for item in db_items:
        # Format the numbers nicely before passing them to the row
        formatted_cost = f"₱ {float(item['unit_cost']):,.2f}"
        formatted_reorder = f"{item['reorder_level']} {item['unit_of_measurement']}"
        
        # Determine status dynamically based on current stock
        ...
        rows.append(
            ingredient_supply_row(
                pid=f"P{item['ingredient_id']:04d}",
                sid=f"S{item['supplier_id']:04d}",
                item=item["item_name"],
                category=item["category"],
                unit=item["unit_of_measurement"],
                cost=formatted_cost,
                reorder_at=formatted_reorder,
                status=status
            )
        )
...
def ingredients_supply_view(page: ft.Page, user, on_logout, on_nav):
    sidebar = build_sidebar(page, user, on_logout, "Ingredients & Supplies", on_nav)
