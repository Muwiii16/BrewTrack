...
def stock_out_usage_view(page: ft.Page, user=None, show_login=None, global_navigate_to=None):
    ...
    reason_dropdown = ft.Dropdown(
        hint_text="Ingredient usage",
        options=[ft.dropdown.Option("Ingredient usage"), ft.dropdown.Option("Waste"), ft.dropdown.Option("Transfer")],
        bgcolor=INPUT_BG, border_color=INPUT_BORDER, focused_border_color=ACCENT, color=TEXT_PRIMARY, border_radius=8, height=45
    )

    recent_list_container = ft.Column(spacing=15)

    def load_recent_stock_outs():
        movements = stock_movement_model.get_recent_movements(limit=10)
        stock_outs = [m for m in movements if m['movement_type'] == 'Stock-Out']
                ...
                ft.Row(
                    ...
                    controls=[
                        ...
                        ft.Column(horizontal_alignment="end", spacing=4, controls=[
                            ft.Text(f"- {float(s['quantity']):g}", size=14, color=ACCENT, weight="bold"),
    ...
    def handle_stock_out(e):
        if not item_dropdown.value or not quantity_field.value or not reason_dropdown.value:
            snack = ft.SnackBar(content=ft.Text("Item, Quantity, and Reason are required."), bgcolor=ft.Colors.RED_800)
        ...
        try:
            inventory_model.stock_out(
                ...
                None,
                f"{reason_dropdown.value} entry"
            )
            # Clear fields
            item_dropdown.value = None
            quantity_field.value = ""
            reason_dropdown.value = None
            
            snack = ft.SnackBar(content=ft.Text("Inventory successfully deducted!"), bgcolor=ft.Colors.GREEN_800)
            page.overlay.append(snack)
            snack.open = True
            load_recent_stock_outs()
    ...
    form_card = ft.Container(
        ...
        content=ft.Column(
            spacing=15,
            controls=[
                ft.Text("Record Usage", size=18, color=TEXT_PRIMARY, weight="bold"),
                ft.Container(height=5),
                ft.Column(spacing=4, controls=[ft.Text("Item *", size=12, weight="bold", color=TEXT_PRIMARY), item_dropdown]),
                ft.Column(spacing=4, controls=[ft.Text("Quantity Used *", size=12, weight="bold", color=TEXT_PRIMARY), quantity_field]),
                ft.Column(spacing=4, controls=[ft.Text("Reason *", size=12, weight="bold", color=TEXT_PRIMARY), reason_dropdown]),
                ft.Container(height=20),
                ft.Container(
                    bgcolor="#332400", padding=ft.Padding.symmetric(vertical=15), border_radius=8, ink=True, on_click=handle_stock_out,
                    content=ft.Row(alignment="center", controls=[ft.Text("Deduct from Inventory", size=13, weight="bold", color=ACCENT)])
                )
            ]
        )
    )
    ...
    recent_card = ft.Container(
        ...
        content=ft.Column(
            spacing=20,
            controls=[
                ft.Text("Recent Stock-Out", size=18, color=TEXT_PRIMARY, weight="bold"),
    ...
    main_content.content = ft.Column(
        ...
        controls=[
            ft.Column(spacing=4, controls=[
                ft.Row(spacing=8, controls=[ft.Icon(ft.Icons.OUTPUT_ROUNDED, size=20, color=TEXT_MUTED), ft.Text("Stock-Out / Ingredient Usage", size=14, color=TEXT_MUTED, weight="bold")]),
                ft.Container(height=10),
                ft.Text("Stock-Out / Ingredient Usage", size=28, weight="bold", color=TEXT_PRIMARY),
                ft.Text("Deduct ingredients used in production, waste, or transfers", size=12, color=TEXT_MUTED),
            ]),
            ft.Row(spacing=20, vertical_alignment="start", controls=[form_card, recent_card])
        ]
    )

    sidebar_container.content = build_sidebar("Stock-Out/ Usage")
    load_recent_stock_outs()
